...
#define BackColor [UIColor colorWithRed:245.0f/255.0f green:245.0f/255.0f blue:245.0f/255.0f alpha:1.0]
...
-(void)setCellText:(NSString *)text align:(NSString*)align{
    ...
    // 只取宽度
    CGSize textSize = [text textSizeWithFont:[UIFont systemFontOfSize:15.0f] constrainedToSize:CGSizeMake(MAXFLOAT, 14) lineBreakMode:NSLineBreakByWordWrapping];
...
- (instancetype)initWithOrigin:(CGPoint)origin andHeight:(CGFloat)height {
    ...
    if (self) {
        ...
        _leftTableView.rowHeight = kWidth *0.125;
        ...
        _rightTableView.rowHeight = kWidth *0.125;
...
- (void)animateCollectionView:(UICollectionView *)collectionView show:(BOOL)show complete:(void(^)())complete {
    ...
    if (show) {
        ...
        if (collectionView) {
            ...
            collectionViewHeight = ([collectionView numberOfItemsInSection:0] > 10) ? (5 * 44) : (ceil([collectionView numberOfItemsInSection:0]/2) * 44);
        }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([self.dataSource respondsToSelector:@selector(menu:titleForRowAtIndexPath:)]) {
        ...
        // 只取宽度
        textSize = [titleLabel.text textSizeWithFont:[UIFont systemFontOfSize:17.0f] constrainedToSize:CGSizeMake(MAXFLOAT, 14) lineBreakMode:NSLineBreakByWordWrapping];
    ...
    if (leftOrRight == 1) {
        ...
        if ([titleLabel.text isEqualToString:[(CATextLayer *)[_titles objectAtIndex:_currentSelectedMenudIndex] string]]) {
            ...
            CGFloat widthX = kWidth *0.7 -30;
            
            accessoryImageView.frame = CGRectMake(widthX, (self.frame.size.height-12)/2, 16, 12);
...
{
    return CGSizeMake((collectionView.frame.size.width-1)/2, 44);
}
